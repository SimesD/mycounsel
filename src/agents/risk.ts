            ...
            items: {
                ...
                properties: {
                    ...
                    impact: {
                        ...
                        description:
                            "HIGH = could void/unenforceable or major litigation risk; MEDIUM = notable gap, manageable; LOW = drafting refinement only",
                    },
                },
                required: ["title", "detail", "statutory_basis", "impact"],
            },
        },
        statutory_basis: { type: Type.STRING },
...
2. Identify ALL REMAINING material weaknesses (not ones already fixed) — no cap on number
3. Note any NEW issues introduced by the revision
4. Assign impact ratings (HIGH / MEDIUM / LOW) to every remaining issue
5. Sort warnings HIGH first, then MEDIUM, then LOW
6. Score the enforceability — the score should INCREASE if issues were resolved
...
Find ALL weaknesses, ambiguities, and legally vulnerable provisions that opposing counsel could exploit.

1. Identify EVERY material legal weakness — do not cap at 3. A well-drafted commercial contract typically has 3–8 issues; a poor one may have more.
2. For each issue assign an impact rating:
   - HIGH: could render the contract void, unenforceable, or expose a party to major litigation risk
   - MEDIUM: notable gap or ambiguity with real commercial consequences
   - LOW: drafting refinement — technically valid but suboptimal
3. Sort warnings HIGH first, then MEDIUM, then LOW
4. Score overall enforceability 0–100
5. State the primary governing statute
6. Give a single actionable recommendation
